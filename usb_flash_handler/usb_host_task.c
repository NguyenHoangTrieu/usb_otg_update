...
esp_err_t usbcdc_send_data(usb_device_t *dev, const uint8_t *data, size_t len, int timeout_ms)
{
    ...
    transfer->bEndpointAddress = dev->dev_addr; // endpoint_out needs to be initialized from descriptor
    ...
    if (err == ESP_OK) {
        ESP_LOGI("USBOTG", "Sent %d bytes to device: endpoint 0x%02X", (int)len, dev->dev_addr);
    } else {
 ...
 *
    ...
    * @param[in] max_len      Maximum number of bytes to receive (size of data buffer)
...
esp_err_t usbcdc_receive_data(usb_device_t *dev, uint8_t *data, size_t max_len, size_t *actual_len)
{
    ...
    err = usb_host_transfer_submit(transfer);
    if (err == ESP_OK) {
        // In sync (poll) case: memory will have been updated immediately; in async/callback, this part
